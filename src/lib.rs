...
impl<I2C, E> Lsm6dsox<I2C> 
    ...
    pub fn read_id(&mut self) -> Result<u8, E> {
        ...
        &mut self.i2c.write_read(SLAVE_ADDRESS, &[WHO_AM_I], &mut buffer)?;
    ...
    pub fn configure_accel(&mut self) -> Result<(), E>{
        ...
        // 0100 10 1 0
        let configuration: u8 = 0x6A;
        &mut self.i2c.write(SLAVE_ADDRESS, &[CTRL1_XL, configuration])?;
        Ok(())
    }
    ...
    pub fn configure_gyro(&mut self) -> Result<(), E> {
        ...
        // FS_125 = 0;
        let configuration: u8 = 0x6C;
        &mut self.i2c.write(SLAVE_ADDRESS, &[CTRL2_G, configuration])?;
        Ok(())
    }
    ...
    pub fn read_accel(&mut self) -> Result<[f32; 3], E> {
        ...
        const MULTI: u8 = 1 << 7;        
        &mut self.i2c.write_read(SLAVE_ADDRESS, &[OUTX_L_A | MULTI], &mut buffer)?;
    ...
    pub fn read_gyro(&mut self) -> Result<[f32; 3], E> {
        ...
        */
        ...
        self.i2c.write_read(SLAVE_ADDRESS, &[OUTX_H_G], &mut buffer)?;
        word = (buffer[0]  as i16) << 8;
    
        self.i2c.write_read(SLAVE_ADDRESS, &[OUTX_L_G], &mut buffer)?;
        word |= buffer[0] as i16;

        gyro_data[0] = (word as f32) * 2000.0/ 32768.0;

        self.i2c.write_read(SLAVE_ADDRESS, &[OUTY_H_G], &mut buffer)?;
        word = (buffer[0]  as i16) << 8;
    
        self.i2c.write_read(SLAVE_ADDRESS, &[OUTY_L_G], &mut buffer)?;
        word |= buffer[0] as i16;

        gyro_data[1] = (word as f32) * 2000.0/ 32768.0;

        self.i2c.write_read(SLAVE_ADDRESS, &[OUTZ_H_G], &mut buffer)?;
        word = (buffer[0]  as i16) << 8;
    
        self.i2c.write_read(SLAVE_ADDRESS, &[OUTZ_L_G], &mut buffer)?;
        word |= buffer[0] as i16;
